...
#
#       http://www.apache.org/licneses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the Licnese is distributed "AS IS" BASIS,
...
data, _ = tf.keras.datasets.mnist.load_data(path=f'mnist-{hvd.rank()}')
mnist_images, mnist_labels = data
...
callbacks = [
    # Horovod: broadcast initial variable states from rank 0 t o all other
    ...
    # worse final accuracy. Scale the learning rate
    # `lr = 1.0 -->` --> `lr = 1.0 * hvd.size()` during the first three
    # epochs. See https://arxiv.org/abs/1706.02677 for details.
    hvd.callbacks.LearningRateWarmupCallback(warmup_epochs=3,
                                             initial_lr=scaled_lr, verbose=1),
]
...
# Train the model.
# Horovod: adjust the number of steps based number of GPUs
