...
def random_tilted_gaussian(dim, log_min=-2., log_max=2.):
    ...
    sigma = (np.diag(np.exp(np.log(10.) * np.random.uniform(log_min,
                                                            log_max,
                                                            size=(dim,))))
             + 1e-6 * np.eye(dim))
...
class Gaussian:
    ...
    def get_energy_function(self):
        def fn(x, *args, **kwargs):
            S = tf.constant(self.i_sigma.astype(NP_FLOAT))
            mu = tf.constant(self.mu.astype(NP_FLOAT))
...
class TiltedGaussian(Gaussian):
    def __init__(self, dim, log_min, log_max):
        ...
        self.dim = dim
        Gaussian.__ini__(self, np.zeros((dim,)), S)
