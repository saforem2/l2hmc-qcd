...
class GaugeNetworkConv2D(layers.Layer):
    ...
    def __init__(
            ...
            k_init: Optional[Union[Callable, str]] = 'zeros',
            **kwargs,
    ):
        ...
        self.gauge_net = GaugeNetwork(config, xdim, factor, k_init, **kwargs)

    def call(self, inputs, training=None):
        """Call the network (forward-pass)."""
        v, x, t = inputs
        # pylint:disable=protected_access
        v = tf.reshape(v, (-1, *self.x_conv_block._config.input_shape))
        x = tf.reshape(x, (-1, *self.v_conv_block._config.input_shape))
